...
class ResidualBlock(vi.VIModule):
    def __init__(self, channels, variational_distribution=MeanFieldNormalVarDist()):
        super(ResidualBlock, self).__init__()
        self.conv1 = vi.VIConv2d(channels, channels, kernel_size=3, padding=1, bias=False, variational_distribution=variational_distribution)
        self.bn1 = nn.BatchNorm2d(channels, track_running_stats=False)
        self.conv2 = vi.VIConv2d(channels, channels, kernel_size=3, padding=1, bias=False, variational_distribution=variational_distribution)
...
class ResNet5(vi.VIModule):
    def __init__(self, num_classes=1000, variational_distribution=MeanFieldNormalVarDist()):
        super(ResNet5, self).__init__()
        self.conv = vi.VIConv2d(3, 64, kernel_size=7, padding=1, bias=False, variational_distribution=variational_distribution)
        self.bn = nn.BatchNorm2d(64, track_running_stats=False)
        self.pool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.resblock1 = ResidualBlock(64, variational_distribution=variational_distribution)
        self.resblock2 = ResidualBlock(64, variational_distribution=variational_distribution)
        self.global_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = vi.VILinear(64, num_classes, variational_distribution=variational_distribution)
...
if __name__ == "__main__":
    ...
    training_data = ImageFolder(root=imagenet_train_path, transform=transform)

    test_data = ImageFolder(root=imagenet_val_path, transform=transform)
    ...
    model = ResNet5(variational_distribution=MeanFieldNormalVarDist(initial_std=1.)).to(device)
