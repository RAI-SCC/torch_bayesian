...
def bnn_with_normal() -> None:
    ...
    mse_value, crps = test(test_dataloader, model, loss_fn)
    end = time.time()
    #print("Done! time consumed:", end - start)
    return mse_value, crps, end - start, best_epoch

def bnn_with_studentt() -> None:
    """Reimplement the pytorch quickstart tutorial with BNNs."""
    # Download training data from open datasets.
    df = pd.read_csv('/home/peihsuan/Desktop/vi/vi/scripts/hourly_average_load.csv', parse_dates=['timestamp'])
    df = df[df['timestamp'].dt.year < 2020]
    ...
    predictive_distribution = StudentTwithDOFPredictiveDistribution()
    loss_fn = vi.KullbackLeiblerLoss(
        predictive_distribution, dataset_size=len(train_data)#, heat=0.1
    )
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
...
if __name__ == "__main__":
    ...
    for i in range(50):
        ...
        print(seed)
        set_all_seeds(seed)
        mse_value_1, crps_1, time_1, epochs_1 = bnn_with_normal()
        print("normal",mse_value_1, crps_1, time_1, epochs_1)
        mse_value_2, crps_2, time_2, epochs_2 = bnn_with_studentt()
        print("studentt",mse_value_2, crps_2, time_2, epochs_2)
        seed_list.append(seed)
        normal_list.append([mse_value_1, crps_1, time_1, epochs_1])
        studentt_list.append([mse_value_2, crps_2, time_2, epochs_2])

    torch.save(seed_list, "seed_list2.pt")
    torch.save(normal_list, "normal_list2.pt")
    torch.save(studentt_list, "studentt_list2.pt")
